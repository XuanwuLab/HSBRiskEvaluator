...
class DiffResult(BaseModel):
    same_project: bool = Field(description="Whether the files are from the same project")
    files : List[DiffFile] = Field(description="List of files with their similarity rates")

class Comparator:
    def __init__(self,github_collector: GitHubRepoCollector=GitHubRepoCollector(), apt_utils: AptUtils=AptUtils()):
        self.github_collector = github_collector
        self.apt_utils = apt_utils
        pass
    def compare_dirs(self, dir1: str, dir2: str) -> DiffResult: 
        ...
        for file in detector.test_files:
            ...
            if detect_language(file) in ("Groff", "Text only", "Unknown"):
                continue
            similarity = next((x for x in similar_list if x[2] == file), [0,])[0]
    ...
    def clone_and_compare(self, package: str) -> DiffResult:
        ...
        return self.compare_dirs(debian_path, github_path)
