    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if not response.upstream_git_url.startswith("http"):
                logger.error(
                    f"Invalid upstream URL format for {package_name}: {response.upstream_git_url}"
                )
                return None
            ...
            if resp.status == 200:
                ...
                logger.error(
                    f"Invalid upstream URL for {package_name}: {response.upstream_git_url}"
                )
                return None

    except ValidationError as e:
        logger.error(
            f"Validation error while fetching upstream info for {package_name}: {e}"
        )
    except Exception as e:
        logger.error(
            f"Unexpected error while fetching upstream info for {package_name}: {e}"
        )
    return None
