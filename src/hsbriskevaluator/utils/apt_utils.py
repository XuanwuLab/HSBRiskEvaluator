...
class AptUtils:
    ...
    def run_command(self, command: List[str]) -> str:
        ...
        try:
            ...
            if result.returncode != 0:
                logger.warning(f"Command {' '.join(command)} failed with return code {result.returncode}")
    ...
    def get_package_by_priority(self, priority: Literal["required", "important", "standard"]) -> List[PackageInfo]:
        """
    ...
    def _get_direct_reverse_dependencies(self, package_name: str) -> Set[str]:
        ...
        try:
            output = self.run_command(["apt", "rdepends", "--no-recommends", "--no-suggests", package_name])
            if not output:
                return set()
            ...
            logger.warning(f"Failed to get direct reverse dependencies for {package_name}: {str(e)}")
            return set()
    ...
    def is_os_default_package(self, package_name: str) -> bool:
        ...
        if package_info.priority and package_info.priority.lower() in ["required", "important"]:
            return True
        
        # Check if package is in base system sections
        if package_info.section:
            base_sections = ["base", "admin", "utils", "shells"]
            if any(section in package_info.section.lower() for section in base_sections):
                return True
    ...
    def is_mainstream_os_package(self, package_name: str) -> bool:
        ...
        if package_info.section:
            common_sections = [
                "devel", "editors", "net", "web", "database", "mail",
                "text", "graphics", "sound", "video", "games", "misc"
            ]
            if any(section in package_info.section.lower() for section in common_sections):
                return True
    ...
    def get_package_classification(self, package_name: str) -> Tuple[bool, bool]:
        ...
        is_mainstream_os = self.is_mainstream_os_package(package_name) if not is_os_default else False
        
        return is_os_default, is_mainstream_os
    
    def estimate_dependency_depth(self, package_name: str, target_packages: Set[str]) -> int:
        ...
        try:
            ...
            logger.warning(f"Failed to estimate dependency depth for {package_name}: {str(e)}")
            return 2  # Default depth

    @lru_cache(maxsize=500)
    def fetch_recursive_dependencies(self, package_name: str) -> List[RecursiveDependency]:
        """
        ...
        try:
            ...
            logger.error(f"Error fetching recursive dependencies for {package_name}: {e.output.decode()}")
            raise RuntimeError(f"Failed to fetch dependencies for {package_name}") from e
        except subprocess.TimeoutExpired:
            ...
            logger.error(f"Unexpected error fetching recursive dependencies for {package_name}: {str(e)}")
            raise RuntimeError(f"Failed to fetch dependencies for {package_name}") from e
    ...
    def _parse_recursive_dependencies(self, apt_output: str) -> List[RecursiveDependency]:
        """
        ...
        for line in lines:
            ...
            if not stripped_line or any(keyword in stripped_line for keyword in ['Reading', 'Building', 'Done']):
                continue
                    ...
                    if current_package:
                        for dep in deps.split(','):
                            ...
                            if dep_match:
                                ...
                                dep_version = dep_match.group(2) if dep_match.group(2) else None
                                dependencies.append(RecursiveDependency(
