...
class CommunityEvaluator(BaseEvaluator):
    ...
    def _count_direct_commit_users(self) -> int:
        ...
        logger.debug(
            f"Estimated direct commit users (approximation): {len(unique_contributors)}"
        )
        ...
        try:
            response = self.llm.invoke(
                prompt.format(
                    ...
                    body=pr.body[:500]
                    if pr.body
                    else "No description",  # Limit body length
                    author=pr.author,
                    status=pr.status,
    ...
    def _analyze_issue_legitimacy_with_llm(self, issue: Issue) -> bool:
        ...
        try:
            ...
            if isinstance(response_content, str):
                ...
                logger.warning(
                    f"Unexpected response type from LLM for issue {issue.number}"
                )
