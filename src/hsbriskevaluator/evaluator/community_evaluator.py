...
from hsbriskevaluator.collector.repo_info import RepoInfo, PullRequest, Issue, Commit
...
class CommunityEvaluator(BaseEvaluator):
    ...
    def _list_direct_commits(self) -> list[Commit]:
        ...
        for pr in self.repo_info.pr_list:
            if pr.status == "merged" and pr.commits:
                for commit in pr.commits:
                    if (commit.committer, commit.message) not in pulls_commits.keys():
                        pulls_commits[(commit.committer, commit.message)] = [
                            commit]
                    else:
                        pulls_commits[(commit.committer, commit.message)].append(
                            commit)
        for commit in self.repo_info.commit_list:
            if (commit.committer, commit.message) not in pulls_commits.keys():
                direct_commits.append(commit)
            elif not any(abs(datetime.fromisoformat(c.timestamp)-datetime.fromisoformat(commit.timestamp)) < timedelta(days=1) for c in pulls_commits[(commit.committer, commit.message)]):
                direct_commits.append(commit)
        return direct_commits

    def _list_maintainers(self) -> list[str]:
        ...
        for pr in self.repo_info.pr_list:
            if pr.status == "merged" and pr.commits:
                for commit in pr.commits:
                    pulls_commits.add(commit.hash)
        for commit in self.repo_info.commit_list:
            if commit.hash not in pulls_commits:
                unique_contributors.add(commit.committer)
        logger.debug(f"Direct commit users: {unique_contributors}")
        return list(unique_contributors)

    def _count_direct_commit_users(self) -> int:
        return len(self._list_maintainers())

    def _list_reviewers(self) -> Set[str]:
    ...
    def _count_pr_reviewers(self) -> int:
        ...
        return len(self._list_reviewers())
    ...
    def _estimated_prs_to_become_maintainer(self) -> float:
        ...
        for user in maintainers:
            first_direct_commit = list(
                filter(lambda commit: commit.committer == user, direct_commits))[-1]
            granted_to_maintainer_date = datetime.fromisoformat(
                first_direct_commit.timestamp)
            ...
            if early_prs == []:
                logger.debug(
                    f"User {user} has no PRs before granted to maintainer")
            else:
                logger.debug(
                    f"User {user} contributed to {len(early_prs)} PRs in {granted_to_maintainer_date - datetime.fromisoformat(early_prs[-1].created_at)} before becoming a maintainer")
            activities.append(len(early_prs))

        logger.debug(
            f"Average time to become maintainer: {mean(activities) if len(activities) else 0.0} PRs")
    ...
    def _estimated_prs_to_become_reviewer(self) -> float:
        ...
        for user in reviewers:
            ...
            direct_commits = list(
                filter(lambda commit: commit.committer == user, direct_commits))
            ...
            if early_prs == []:
                logger.debug(
                    f"User {user} has no PRs before granted to reviewer")
            else:
                logger.debug(
                    f"User {user} contributed to {len(early_prs)} PRs in {granted_to_reviewer_date - datetime.fromisoformat(early_prs[-1].created_at)} before becoming a reviewer")
            activities.append(len(early_prs))
        logger.debug(
