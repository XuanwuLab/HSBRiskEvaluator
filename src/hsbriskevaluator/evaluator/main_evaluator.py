...
class HSBRiskEvaluator(BaseEvaluator):
    ...
    def evaluate(self) -> EvalResult:
        ...
        logger.info(f"Starting comprehensive HSB risk evaluation for repository: {self.repo_info.repo_id}")
        
        try:
            # Run evaluations concurrently for better performance
            community_result, payload_result, dependency_result = self._run_evaluations_concurrently()
    ...
    def _calculate_overall_risk_score(
        ...
        logger.debug(f"Risk scores - Community: {community_risk:.3f}, "
    ...
    def get_risk_summary(self, result: EvalResult) -> dict:
        ...
        summary = {
            ...
            "community_quality": {
                ...
                "level": risk_level(self._calculate_community_risk_score(result.community_quality)),
            ...
            "payload_hiding": {
                ...
                "key_concerns": self._get_payload_concerns(result.payload_hidden_difficulty)
            },
            "dependency_risk": {
    ...
    def _get_payload_concerns(self, payload_result) -> list:
        ...
        if len(payload_result.suspicious_binary_files) > 0:
            concerns.append(f"{len(payload_result.suspicious_binary_files)} suspicious binary files detected")
